...
struct Puzzle {
    int    number;
    string privHex;       // known private-key hex (or empty)
    string address;       // expected address
};
...
string bytesToHex(const vector<unsigned char>& bytes) {
    ...
    for (auto b : bytes) oss << setw(2) << (int)b;
    return oss.str();
}

// Base58 encoding (no external dependencies)
...
string base58Encode(const vector<unsigned char>& input) {
    ...
    size_t idx = 0; while (idx < b58.size() && b58[idx] == 0) ++idx;
    string result;
    result.reserve(zeros + (b58.size()-idx));
    result.assign(zeros, '1');
    for (; idx < b58.size(); ++idx) result += BASE58_ALPHABET[b58[idx]];
    return result;
}

// Generate P2PKH address from private-key hex
string privHexToAddress(const string& privHex) {
    // Convert to BIGNUM
    BIGNUM* bn = BN_new();
    BN_hex2bn(&bn, privHex.c_str());
    ...
    unsigned char pubbuf[33],*p = pubbuf;
    int publen = i2o_ECPublicKey(ec, &p);

    unsigned char sha256d[SHA256_DIGEST_LENGTH];
    SHA256(pubbuf, publen, sha256d);

    unsigned char ripemd160d[RIPEMD160_DIGEST_LENGTH];
    RIPEMD160(sha256d, SHA256_DIGEST_LENGTH, ripemd160d);

    vector<unsigned char> payload(1 + RIPEMD160_DIGEST_LENGTH);
    payload[0] = 0x00;
    memcpy(payload.data()+1, ripemd160d, RIPEMD160_DIGEST_LENGTH);

    unsigned char checksum1[SHA256_DIGEST_LENGTH], checksum2[SHA256_DIGEST_LENGTH];
    SHA256(payload.data(), payload.size(), checksum1);
    SHA256(checksum1, SHA256_DIGEST_LENGTH, checksum2);

    payload.insert(payload.end(), checksum2, checksum2+4);
    string address = base58Encode(payload);
...
string bytesToWIF(const vector<unsigned char>& privBytes) {
    vector<unsigned char> data{0x80};
    data.insert(data.end(), privBytes.begin(), privBytes.end());
    data.push_back(0x01); // compressed flag

    unsigned char d1[SHA256_DIGEST_LENGTH], d2[SHA256_DIGEST_LENGTH];
    SHA256(data.data(), data.size(), d1);
    SHA256(d1, SHA256_DIGEST_LENGTH, d2);

    data.insert(data.end(), d2, d2+4);
...
void printHeader() {
    cout << "#   "
         << setw(PRIV_HEX_WIDTH) << left  << "PrivateKey" << ' '
         << setw(ADDR_WIDTH)    << left  << "Address"    << ' '
         << setw(WIF_WIDTH)     << left  << "WIF"        << ' '
         << setw(PCT_WIDTH)     << right << "%Range"     << ' '
         << "Status" << '\n';
    cout << string(4 + PRIV_HEX_WIDTH + 1 + ADDR_WIDTH + 1 + WIF_WIDTH + 1 + PCT_WIDTH + 1 + 6, '-') << '\n';
}
...
int main(int argc, char* argv[]) {
    ...
    // Context for HMAC
    const unsigned char* key = reinterpret_cast<const unsigned char*>(argv[2]);
    int keylen = strlen(argv[2]);
    BN_CTX* ctx = BN_CTX_new();

    printHeader();
    for (int i = 1; i <= count; ++i) {
        // Derive deterministic BIGNUM via HMAC-SHA256
        unsigned char idx[2] = { static_cast<unsigned char>(i>>8), static_cast<unsigned char>(i&0xff) };
        unsigned char digest[SHA256_DIGEST_LENGTH];
        unsigned int len = 0;
        HMAC(EVP_sha256(), key, keylen, idx, 2, digest, &len);

        BIGNUM* bn = BN_bin2bn(digest, len, nullptr);
        BN_mask_bits(bn, i);
        BN_set_bit(bn, i-1);
        ...
        string privHex = bytesToHex(privBytes);

        // Compute address and WIF
        string addr = privHexToAddress(privHex);
        string wif  = bytesToWIF(privBytes);

        // Check match
        bool match = false;
        const Puzzle& p = puzzles[i-1];
        if (!p.privHex.empty()) {
            match = (privHex == p.privHex);
            addr  = p.address; // use expected
        } else {
            match = (!p.address.empty() && addr == p.address);
        }

        // Compute percentage of range
        BIGNUM* minBN = BN_new(); BN_set_bit(minBN, i-1);
        BIGNUM* maxBN = BN_new(); BN_set_bit(maxBN, i); BN_sub_word(maxBN,1);
        BIGNUM* rangeBN = BN_new(); BN_sub(rangeBN, maxBN, minBN);
        BIGNUM* offsetBN = BN_new(); BN_sub(offsetBN, bn, minBN);
        BN_mul_word(offsetBN, 10000);
        BIGNUM* pctBN = BN_new(); BN_div(pctBN, nullptr, offsetBN, rangeBN, ctx);
        unsigned long q = BN_get_word(pctBN);
        unsigned pctWhole = q / 100, pctFrac = q % 100;
        ostringstream pctOSS;
        pctOSS << pctWhole << '.' << setw(2) << setfill('0') << pctFrac << '%';

        // Cleanup BNs
        BN_free(minBN); BN_free(maxBN); BN_free(rangeBN);
        BN_free(offsetBN); BN_free(pctBN); BN_free(bn);

        if (hideNonMatches && !match) continue;

        // Print line
        cout << setw(3) << right << i << " "
             << setw(PRIV_HEX_WIDTH) << left  << privHex << ' '
             << setw(ADDR_WIDTH)    << left  << addr    << ' '
             << setw(WIF_WIDTH)     << left  << wif     << ' '
             << setw(PCT_WIDTH)     << right << pctOSS.str() << ' '
             << (match ? ansi::GREEN : ansi::RED)
             << (match ? "MATCH" : "FAIL")
             << ansi::RESET << '\n';
    }
